...
export function toDb(project) {
  return {
    ...
    project_start:  project.projectStart  || null,
    project_finish: project.projectFinish || null,
    actual_finish:  project.actualFinish  || null,
    kit: project.kit ?? null,
    milestone: project.milestone ?? 0,
    milestones_list: project.milestonesList ?? null,
    on_hold: project.onHold ?? false,
...
export function fromDb(row) {
  return {
    ...
    projectFinish:  row.project_finish ?? null,
    actualFinish:   row.actual_finish  ?? null,
    kit:            row.kit ?? null,
    milestone:      row.milestone ?? 0,
    milestonesList: row.milestones_list ?? null,
